 ...
 * @Description: TODO(演示例子处理类 和 流程处理类型)
 * @author jeecg
 */
@Scope("prototype")
@Controller
...
public class DemoController extends BaseController {
	...
	public ModelAndView aorudemo(TSFlowType demo, HttpServletRequest request) {
		String type = oConvertUtils.getString(request.getParameter("type"));
		if (demo.getId() != null) {
			demo = systemService.getEntity(TSFlowType.class, demo.getId());
	...
	public List<ComboTree> pDemoList(HttpServletRequest request, ComboTree comboTree) {
		CriteriaQuery cq = new CriteriaQuery(TSFlowType.class);
		...
		cq.add();
		List<TSFlowType> demoList = systemService.getListByCriteriaQuery(cq, false);
	...
	public String demoTurn(String id){
		String code = systemService.get(TSFlowType.class, id).getDemocode();
	 ...
	 * 流程类型管理
	 */
	@RequestMapping(params = "flowTypeList")
	public ModelAndView flowTypeList(HttpServletRequest request) {
		return new ModelAndView("workflow/flowtype/flowTypeList");
	}
	
	/**
	 * 流程添加页面跳转
	 */
	@RequestMapping(params = "aoruFlowType")
	public ModelAndView aoruFlowType(TSFlowType demo, HttpServletRequest request) {
		String type = oConvertUtils.getString(request.getParameter("type"));
		if (demo.getId() != null) {
			demo = systemService.getEntity(TSFlowType.class, demo.getId());
			request.setAttribute("demo", demo);
		...
		if (type.equals("table")) {
			...
			return new ModelAndView("workflow/flowtype/flowTypeAdd");
		}
	 ...
	 * 流程类型修改页面跳转
	 */
	@RequestMapping(params = "aoruflowTypemo")
	public ModelAndView aoruflowTypemo(TSFlowType demo, HttpServletRequest request) {
		String type = oConvertUtils.getString(request.getParameter("type"));
		if (demo.getId() != null) {
			demo = systemService.getEntity(TSFlowType.class, demo.getId());
			request.setAttribute("demo", demo);
	...
	public ModelAndView demoCode(TSFlowType demo, HttpServletRequest request) {
		List<TSFlowType> list = systemService.getList(TSFlowType.class);
		demo = list.get(0);
		request.setAttribute("demo", demo);
	...
	@ResponseBody
	public AjaxJson saveDemo(TSFlowType demo, HttpServletRequest request) throws Exception{
		AjaxJson j = new AjaxJson();
		if (!StringUtil.isEmpty(demo.getId())) {
			message = "Demo维护例子: " + demo.getDemotitle() + "被更新成功";
			TSFlowType entity = this.systemService.get(TSFlowType.class, demo.getId());
	...
	@ResponseBody
	public AjaxJson del(TSFlowType demo, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		demo = systemService.getEntity(TSFlowType.class, demo.getId());
